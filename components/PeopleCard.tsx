...
const PeopleCard = ({ person }: any) => {
  ...
  return (
    <View style={styles.cardContainer}>
      <TouchableOpacity style={{flex:1,flexDirection: 'row',alignItems:'flex-end'}} onPress={()=>navigation.navigate(routes.CustomerInformation)}>
        <Image
          source={person.image}
        ...
        <View style={{justifyContent: 'space-between', paddingLeft: 5}}>
          ...
          <Text style={styles.text2}>{person.Mortgagetype}</Text>
					...
					<TouchableOpacity style={styles.buttonRepaymentschedule} onPress={()=>navigation.navigate(routes.Repaymentschedule)}>
...
const styles = StyleSheet.create({
  ...
  buttonRepaymentschedule: {
    flex:1, backgroundColor: COLORS.sky,height:22,width:45,marginStart:10,alignItems:'center',justifyContent:'center',borderRadius:2
  }
});
